...
class ImportTest extends Specification {
   ...
   def "Execute :import task with defaults"() {
      given:
      taskName = 'import'
      result = executeSingleTask(taskName, ['-Si','-Djavax.xml.parsers.SAXParserFactory=oracle.xml.jaxp.JXSAXParserFactory'])
   ...
   def "Execute :importProjectDir task with --source-dir option"() {
      given:
      taskName = 'importProjectDir'
      result = executeSingleTask(taskName, ['--source-dir=EDW Loads', '-Si','-Djavax.xml.parsers.SAXParserFactory=oracle.xml.jaxp.JXSAXParserFactory'])
